...
- **[GCP_PLATFORM_INTEGRATION.md](GCP_PLATFORM_INTEGRATION.md)** - GCP platform integration guide
  - IAM architecture: two service accounts (node SA + operator SA)
  - Custom IAM role with 103 granular permissions
  - Credential injection into 7 operator namespaces
  - CCM taint removal (chicken-and-egg problem)
  - LoadBalancer services and firewall rules
  - GCP Persistent Disk CSI (RWO)
  - GCP Filestore CSI (RWX NFS)
  - DNS zone patching, Machine API cleanup, teardown

### Post-Bootstrap Operations
- **[POST_BOOTSTRAP_GUIDE.md](POST_BOOTSTRAP_GUIDE.md)** - From bootstrapped to production-ready
  - Automated steps: credential injection, CCM taint, Filestore, LB
  - Manual steps: DNS update, zone patching, Machine API cleanup
  - Verification at each stage + final health check
...
1. Automated steps: run `./deploy.sh` (sections 8-10b happen automatically)
2. Manual steps: [POST_BOOTSTRAP_GUIDE.md](POST_BOOTSTRAP_GUIDE.md#part-2-manual-steps-you-must-do-these)
3. Final health check: [POST_BOOTSTRAP_GUIDE.md](POST_BOOTSTRAP_GUIDE.md#final-health-check)
...
| LoadBalancer not provisioning | [GCP_PLATFORM_INTEGRATION.md](GCP_PLATFORM_INTEGRATION.md#7-loadbalancer-services) |
| Filestore PVC stuck pending | [GCP_PLATFORM_INTEGRATION.md](GCP_PLATFORM_INTEGRATION.md#9-storage-gcp-filestore-csi-rwx-nfs) |
| CCM taint blocks pod scheduling | [GCP_PLATFORM_INTEGRATION.md](GCP_PLATFORM_INTEGRATION.md#6-ccm-taint--chicken-and-egg-problem) |
| Operator degraded (missing credentials) | [GCP_PLATFORM_INTEGRATION.md](GCP_PLATFORM_INTEGRATION.md#5-credential-injection-pipeline) |
| Cluster bootstrapped but operators degraded | [POST_BOOTSTRAP_GUIDE.md](POST_BOOTSTRAP_GUIDE.md) |
| Teardown fails (orphaned LB resources) | [GCP_PLATFORM_INTEGRATION.md](GCP_PLATFORM_INTEGRATION.md#12-teardown-considerations) |
