...
│   ├── POST_BOOTSTRAP_GUIDE.md  # Post-bootstrap operations guide
...
2. **Enable Required APIs:**
   ...
   gcloud services enable file.googleapis.com    # Required for Filestore CSI (RWX storage)
   ```
...
3. **Configure Permissions:**
   ...
   For `platform: gcp` with `credentialsMode: Manual`, see **[GCP_PLATFORM_INTEGRATION.md](docs/GCP_PLATFORM_INTEGRATION.md)** for:
   - Two service accounts (node SA + operator SA) with a custom IAM role
   - Credential injection into 7 operator namespaces
   - CCM taint removal and LoadBalancer setup
...
4. **Infrastructure**: Deploys VMs, networks, DNS, operator SA with Terraform
...
8. **Credentials**: Injects operator SA key into 7 operator namespaces
9. **CCM Taint**: Removes cloud-provider uninitialized taint from all nodes
10. **Filestore**: Enables Filestore CSI driver and creates RWX StorageClass
11. **LoadBalancer**: Waits for ingress LB to provision
12. **Validation**: Verifies cluster health
...
- **[GCP_PLATFORM_INTEGRATION.md](docs/GCP_PLATFORM_INTEGRATION.md)** - GCP platform integration guide
  - IAM architecture (node SA + operator SA with 103-permission custom role)
  - Credential injection into operator namespaces
  - CCM taint removal, LoadBalancer, storage (PD CSI + Filestore CSI)
  - DNS zone patching, Machine API cleanup, teardown procedures

- **[POST_BOOTSTRAP_GUIDE.md](docs/POST_BOOTSTRAP_GUIDE.md)** - Post-bootstrap operational runbook
  - From bootstrapped MVP to fully functional cluster
  - Credential injection, DNS, LoadBalancer, storage, Machine API cleanup
...
4. After bootstrap: [POST_BOOTSTRAP_GUIDE.md](docs/POST_BOOTSTRAP_GUIDE.md)
5. If issues arise: [DEBUG_COMMANDS.md](docs/DEBUG_COMMANDS.md)
...
- **Compute Instances**: 1 bootstrap, 3 control planes, 2 workers, 1 bastion
- **DNS**: Private managed zone with required A records
- **Storage**: GCS bucket for bootstrap ignition files
- **IAM**: Node SA (instance metadata) + Operator SA (secret injection) with custom role (103 permissions)

### OpenShift Cluster
- **Version**: OpenShift 4.19
- **Networking**: OVN-Kubernetes (default)
- **Storage**: GCP Persistent Disk CSI (RWO) + Filestore CSI (RWX NFS)
- **High Availability**: 3 control planes across zones
- **Workers**: 2 worker nodes for application workloads
...
- **[GCP_PLATFORM_INTEGRATION.md](docs/GCP_PLATFORM_INTEGRATION.md)** - If changing platform integration (IAM roles, credentials, storage, LB)
